...
import com.example.model.FSA;
import com.example.model.LambdaState;
...
public class FSACreator {
    public static FSA generateAFD (DoubleNode start, String transSymbols)
    {
        ...
        FSA fsa = new FSA();
        fsa.symbols = transSymbols;
        fsa.states = createStates(uptakingNode, lamdaTransitions, transSymbols, nodes);
        
        return fsa;
    }
    
    public static List<LambdaState> createStates (int uptakingNode, List<List<Integer>> lamdas, String transSymbols, List<DoubleNode> nodes)
    {
        List<LambdaState> states = new LinkedList<>();
        
        //If contains uptaking node tells it to be an uptaking state.
        LambdaState state = new LambdaState(lamdas.get(0), lamdas.get(0).contains(uptakingNode));
        states.add(state);
        
        for (int x = 0; x < states.size(); x++)
        {
            LambdaState st = states.get(x);
    ...
    String transition,
     LambdaState state,
    List<LambdaState> states,
    List<List<Integer>> lamdaTrans)
    {
        LambdaState createdState = new LambdaState();
//        System.out.println("---------------------------------");
...
//                System.out.print(nodes.get(node - 1).link1.data + ", ");
                ...
                for (Integer trans : transitions) {
                    if (!createdState.lamdaTransitions.contains(trans))
                    {
                        ...
                        createdState.isUptakingState = trans == uptakingNode || createdState.isUptakingState;
                    }
                }
            }
...
//            System.out.println(stateNum);
    ...
    public static int insertState (LambdaState state, List<LambdaState> states)
    {
